...
你有一个 search_similar 工具，能语义搜索历史数据。你应该**主动使用**它，而不是等用户要求。

### 必须主动调用的场景
- 用户提到"之前的脚本""上次的""类似的" → 调用 search_similar(type='scripts')
- 用户说"哪种标题好""爆款标题""互动高的" → 调用 search_similar(type='patterns')
- 用户说"这个创作者的内容模式""竞品在做什么" → 调用 search_similar(type='content')
- 用户要你分析一个选题但你没有足够数据支撑 → 先搜再分析

### 不要调用的场景
- 用户只是要生成新内容（用 expand_script）
- 用户要爬新数据（用 crawl_data）
- 上下文中已经有足够的历史数据（不需要重复搜索）

### 调用方式
- query 用自然语言描述你要找什么，不要太短（"华为手机评测"比"手机"好）
- type 根据场景选择：scripts / content / patterns
- 结果中分数高的内容要在回复中引用，说明来源`

async function buildSystemPrompt(opts = {}) {
  const { accountId = 'default', contentStyle = '', scriptDuration = '', defaultLanguage = '',
          accountName = '', accountPlatform = '', accountUid = '', _suggestingMode = false } = opts
  const cacheKey = PROMPT_CACHE_KEY + ':' + [accountId, contentStyle, scriptDuration, defaultLanguage, accountName, accountPlatform, accountUid, _suggestingMode ? 'suggest' : ''].join('|')
...
class Agent {
  constructor(config) {
    ...
    this._suggestingMode = config._suggestingMode || false
  ...
  async run(userInput, options = {}) {
    ...
    const systemPrompt = await buildSystemPrompt({
      ...
      _suggestingMode: this._suggestingMode,
    })
    this.resetUsage()
    let iterations = 0
      ...
      switch (nextAction.type) {
        case 'execute_tools': {
          ...
          for (const block of nextAction.tools) {
            ...
            processToolResult(block.name, result, state, this.accountId)
